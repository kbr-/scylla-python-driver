    ...
    def return_connection(self, connection, stream_was_orphaned=False):
        ...
        if connection.is_defunct or connection.is_closed:
            ...
            if not connection.signaled_error:
                log.info("HostConnection Defunct or closed connection (%s) returned to pool, potentially "
                ...
                log.info("HostConnection Host %s is_down=%s", self.host, is_down)
                connection.signaled_error = True

            if self.shutdown_on_error and not is_down:
                log.info("HostConnection Host %s shutdown_on_error, wasn't down, scheduling on_down", self.host)
                is_down = True
...
class HostConnectionPool(object):
    ...
    def return_connection(self, connection, stream_was_orphaned=False):
        ...
        if connection.is_defunct or connection.is_closed:
            if not connection.signaled_error:
                log.info("HostConnectionPool Defunct or closed connection (%s) returned to pool, potentially "
                          "marking host %s as down", id(connection), self.host)
                is_down = self._session.cluster.signal_connection_failure(
                    self.host, connection.last_error, is_host_addition=False)
                connection.signaled_error = True
                log.info("HostConnectionPool Host %s is_down=%s", self.host, is_down)
                if is_down:
                    self.shutdown()
                else:
